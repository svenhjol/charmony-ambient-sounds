...
public class Ocean implements SoundType<BiomeSound> {
    public final SoundEvent daySound;
    public final SoundEvent nightSound;

    public Ocean() {
        daySound = SoundEvent.createVariableRangeEvent(ResourceLocation.fromNamespaceAndPath(AmbientSoundsMod.ID, "biome.ocean.day"));
        nightSound = SoundEvent.createVariableRangeEvent(ResourceLocation.fromNamespaceAndPath(AmbientSoundsMod.ID, "biome.ocean.night"));
    }
    ...
    public void addSounds(SoundHandler<BiomeSound> handler) {
        ...
        handler.getSounds().add(new SurfaceBiomeSound(handler.getPlayer(), true) {
            ...
            public boolean isValidBiomeCondition(Holder<Biome> holder, ResourceKey<Biome> key) {
                return BiomeCheckHelper.OCEAN.test(holder);
            }
        });

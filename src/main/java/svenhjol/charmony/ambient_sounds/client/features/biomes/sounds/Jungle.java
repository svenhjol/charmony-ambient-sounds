...
public class Jungle implements SoundType<BiomeSound> {
    public final SoundEvent daySound;
    public final SoundEvent nightSound;

    public Jungle() {
        daySound = SoundEvent.createVariableRangeEvent(ResourceLocation.fromNamespaceAndPath(AmbientSoundsMod.ID, "biome.jungle.day"));
        nightSound = SoundEvent.createVariableRangeEvent(ResourceLocation.fromNamespaceAndPath(AmbientSoundsMod.ID, "biome.jungle.night"));
    }
    ...
    public void addSounds(SoundHandler<BiomeSound> handler) {
        ...
        handler.getSounds().add(new SurfaceBiomeSound(handler.getPlayer(), true) {
            ...
            public boolean isValidBiomeCondition(Holder<Biome> holder, ResourceKey<Biome> key) {
                return BiomeCheckHelper.JUNGLE.test(holder);
            }
        });

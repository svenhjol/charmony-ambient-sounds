...
public class Savanna implements SoundType<BiomeSound> {
    public final SoundEvent daySound;
    public final SoundEvent nightSound;

    public Savanna() {
        daySound = SoundEvent.createVariableRangeEvent(ResourceLocation.fromNamespaceAndPath(AmbientSoundsMod.ID, "biome.savanna.day"));
        nightSound = SoundEvent.createVariableRangeEvent(ResourceLocation.fromNamespaceAndPath(AmbientSoundsMod.ID, "biome.savanna.night"));
    }
    ...
    public void addSounds(SoundHandler<BiomeSound> handler) {
        ...
        handler.getSounds().add(new SurfaceBiomeSound(handler.getPlayer(), false) {
            ...
            public boolean isValidBiomeCondition(Holder<Biome> holder, ResourceKey<Biome> key) {
                return BiomeCheckHelper.SAVANNA.test(holder);
            }
        });
